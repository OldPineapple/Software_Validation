...
class TestChangeProject(unittest.TestCase):
    ...
    def test_add_1(self):
        ...
        t2_start = time.time()
        put_response = requests.put('http://localhost:4567/projects/' + str(num), json=item)
        print(put_response.text)
        t2 = time.time() - t2_start
        get_response = requests.get('http://localhost:4567/projects/' + str(num))
        parsed = json.loads(get_response.text)
        self.assertEqual('new_title', parsed['projects'][0]['title'])
        num = 100
