...
class TestChangeTodo(unittest.TestCase):
    ...
    def setUp(self):
        ...
        global t1_start
        t1_start = time.time()
        populate_todos.populate_todos(num, title_length)
        print("------------------------")
    ...
    def test_add_1(self):
        ...
        t2_start = time.time()
        put_response = requests.put('http://localhost:4567/todos/' + str(num), json=item)
        print(put_response.text)
        t2 = time.time() - t2_start
        get_response = requests.get('http://localhost:4567/todos/' + str(num))
        parsed = json.loads(get_response.text)
        self.assertEqual('new_title', parsed['todos'][0]['title'])
        num = 100
